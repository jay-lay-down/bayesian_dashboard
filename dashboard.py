...
import dash  import io

try:
    from config import _flow_scale, DEFAULT_PATH
    print("✅ 비공개 설정(config.py)을 성공적으로 로드했습니다.")
except ImportError:
    print("⚠️ 경고: config.py 파일을 찾을 수 없습니다. 공개용 기본값으로 대체합니다.")
    print("   (GitHub에 올릴 때는 이 상태가 정상입니다. 로컬 실행 시 config.py를 생성하세요.)")

    # config.py가 없을 때 사용할 안전한 기본값 (공개되어도 괜찮은 최소 기능만 정의)
    DEFAULT_PATH = ""  # 기본 경로를 비워 사용자가 직접 입력하도록 유도

    def _flow_scale(seg, mod, loy) -> float:
        """공개용 기본 스케일 함수 (고정값 1.0 반환)"""
        return 1.0
# --- [설정 분리 끝] ---
    ...
    [
        ...
        html.Div(
            ...
            style={
                ...
                "gap":"18px",
                "padding":"50px 50px 50px", "marginBottom":"36px"
            },
        ),
        ...
        html.Div(
            ...
            style={"display":"grid","gridTemplateColumns":"1fr 1fr 1fr","gap":"18px","padding":"50px 50px 50px", "marginTop":"36px"},
        ),
